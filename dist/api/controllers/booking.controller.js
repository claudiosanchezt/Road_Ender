...
class BookingController {
    static async list(req, res) {
        const items = await booking_service_1.BookingService.list();
    ...
    static async get(req, res) {
        ...
        const item = await booking_service_1.BookingService.get(id);
        if (!item)
    ...
    static async create(req, res) {
        try {
            ...
            const created = await booking_service_1.BookingService.create(body);
    ...
    static async myBookings(req, res) {
        try {
            ...
            const token = authHeader.split(' ')[1];
            const decoded = require('jsonwebtoken').verify(token, process.env.JWT_SECRET || 'supersecret');
            const items = await booking_service_1.BookingService.listByClient(decoded.id);
            res.json({ data: { bookings: items || [] } });
        }
        catch (err) {
            res.status(400).json({ error: err.message || 'Bad request' });
    ...
    static async update(req, res) {
        ...
        if (Number.isNaN(id))
            return res.status(400).json({ error: 'Invalid id' });
        const updated = await booking_service_1.BookingService.update(id, req.body);
        if (!updated)
            return res.status(404).json({ error: 'Booking not found' });
    ...
    static async remove(req, res) {
        ...
        if (Number.isNaN(id))
            return res.status(400).json({ error: 'Invalid id' });
        const ok = await booking_service_1.BookingService.remove(id);
        if (!ok)
            return res.status(404).json({ error: 'Booking not found' });
        res.status(204).send();
    }
